<template>
	<view class="Box" style="">
		...
		<view class="sview" style="color: rgb(0, 0, 0); font-size: 55px; line-height: 27px;">
			...
			<loayImg v-if="!status" v-for="(item,index) in content_text" :key="index" style="width: 100vw;height:1100rpx;" :imgUrl="item.pic" :fill="'scaleToFill'"></loayImg>
			<view v-if="status === 2" class="padding flex flex-direction" style="margin-top:200rpx;">
				<text class="text-sm text-center text-gray">书币不足以扣除该章节</text>
				<button class="cu-btn bg-gradual-orange text-white line-grey round margin-tb-sm lg" data-url="/pages/two/comic/moreSend/moreSend" @tap="toCharge">充值</button>
			</view>
			<view v-if="status === 1" class="padding flex flex-direction" style="margin-top:200rpx;">
				<text class="text-sm text-center text-gray">该章节为付费章节，请登录</text>
				<button class="cu-btn bg-gradual-orange text-white line-grey round margin-tb-sm lg" data-url="/pages/two/comic/moreSend/moreSend" @tap="toCharge">登录</button>
			</view>
		</view>
		<!-- 小说正文结束 -->
		<!-- start抽屉 -->
