	...
	<view>
		...
		<view class="bg-white solid-top margin-bottom" v-else>
			<view class="cu-bar solid-bottom">
				...
				<view class="action">
					<text @tap="orderClick">{{order}}</text>
				</view>
			</view>
			<view class="cu-list menu">
				<view class="cu-item arrow" v-for="(item,index) in listData" :key="index" @tap="details(item,index)">
					<view class="content">
						{{item.name}}
					</view>
				</view>
		...
		<view class="cu-bar bg-white tabbar border zai-tabbar-bar">
			<view class="btn-group">
				<button class="cu-btn bg-orange round shadow-blur" @tap="AddClick" v-if="addBtn==false">加入书架</button>
				<text v-else>已加入书架</text>
			</view>
			<view class="btn-group">
				<button class="cu-btn bg-red round shadow-blur" @tap="readClick">立即阅读</button>
			</view>
		</view>
	</view>
</template>
